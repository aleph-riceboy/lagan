...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
  ...
  if (!hasValidTxIns) {
    console.log('Some txIns are invalid in tx: ' + transaction.id);
    return false;
  }
...
const validateBlockTransactions = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[], blockIndex: number): boolean => {
  ...
  //Checks for duplicate txIns. Each txIn can ony be included only once
...
const validateCoinbaseTx = (transaction: Transaction, blockIndex: number): boolean => {
  ...
  if (transaction.txIns[0].txOutIndex !== blockIndex) {
    console.log('The txIn signature in coinbase tx must be the block heigth');
    return false;
  }
  if (transaction.txOuts.length !== 1) {
    console.log('Invaild coinbase amount in coinbase transaction');
    return false;
  }
  return true;
};
...
const getTxInAmount = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number => {
  return findUnspentTxOuts(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts).amount;
};

const findUnspentTxOuts = (transactionId: string, index: number, aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut => {
...
const signTxIn = (transaction: Transaction, txInIndex: number,
  ...
  if (getPublicKey(privateKey) !== referencedAddress) {
    console.log('Trying to sign an input with a privatekey that does not match the address that is referenced in txIn');
    throw Error();
  }
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
  ...
  const consumedTxOuts: UnspentTxOut[] = aTransactions
    .map((t) => t.txIns)
    .reduce((a, b) => a.concat(b), [])
    .map((txIn) => new UnspentTxOut(txIn.txOutIde, txIn.txOutIndex, '', 0));
